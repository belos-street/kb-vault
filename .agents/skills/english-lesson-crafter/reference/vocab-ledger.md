...
| **复现率** | **分子 = 本讲之前已入账 ∩ 本讲课文中出现的词条**（词形归并到词条，如 checks / checking 均归并到 check，只计 1）；**分母 = 本讲课文的全部词形（type，仅字母词）**。本讲新入账词不计入分子（它们必然出现，计入即放水）。自各册 L02 起统计，L01 标 N/A（首讲）。分子 / 分母须**脚本或逐词精确清点，不得估算**；台账登记原始值（如 `13/81`），百分比由原始数计算、不得直接填写 |
...
- 复现率按册递进：**第一册常规讲 ≥ 10%（词群实战课 ≥ 20%）、第二册 ≥ 20%、第三册 ≥ 30%**（type 口径、词形归并到词条、分子分母精确清点，自各册 L02 起统计；首讲豁免）
