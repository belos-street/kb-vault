...
| **计算复杂度** | O(n²) | O(n) | O(n) |
| **适用场景** | NLP、多模态 | 序列建模（历史主流，已被 Transformer 取代） | 图像处理 |
...
图像/视频生成的主流方案是**扩散模型（Diffusion Model）**，核心思想：学会"从噪声逐步还原图像"。

- **训练**：给图片逐步加噪声直到纯噪声，训练网络学会预测每一步加入的噪声
- **生成**：从纯随机噪声出发，迭代去噪几十步，图像逐渐"显影"
- **文字控制**：Prompt 经文本编码器（CLIP/T5）编码后，通过 cross-attention 引导去噪方向
- **提速关键**：Latent Diffusion 用 VAE 先压缩到潜空间（如 512×512×3 → 64×64×4），扩散全程在低维空间完成

```mermaid
flowchart LR
    A["纯随机噪声"] --> B["去噪网络预测噪声"]
    B --> C["减去噪声（迭代 20~50 步）<br/>cross-attention 注入文本引导"]
    C --> D["VAE 解码"]
    D --> E["清晰图像"]
```

**视频生成**（Sora、可灵等）= 把扩散从"空间"扩展到"空间 + 时间"：

- 视频压缩后切成**时空 patch**（类比 LLM 的 token），用 **DiT（Diffusion Transformer）** 做跨帧去噪
- 关键难点是**时间一致性**：物体跨帧不消失、不变形，靠跨帧注意力保证
- 两条路线：**扩散**（整段视频同时去噪，画质好，当前主流）vs **自回归**（逐帧预测，类似 GPT 逐 token，适合世界模型方向）

> 联系上表：图像/视频生成的去噪网络已从 U-Net 转向 Transformer 架构（DiT），这正是 Transformer 适用场景标注"多模态"的原因。
