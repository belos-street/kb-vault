...
> Agent 领域的大量"新概念"，骨架大多能在前端、后端、操作系统的既有设计中找到原型。有工程背景的读者可先用旧知识建立映射，再聚焦真正的新东西。

| Agent 黑话 | 一句话含义 | 经典原型 | 出处 |
|-----------|-----------|---------|------|
| Agent（LLM + Memory + Tools） | 自主运行的任务执行体 | 进程 = CPU 调度 + 内存 + I/O | 操作系统 |
| ReAct 循环 | 思考 → 行动 → 观察，循环到任务完成 | 事件循环：取事件 → 处理 → 继续取 | 前端 / Node.js |
| Function Calling | LLM 报函数名和参数，应用层代为执行 | 系统调用：用户态报 syscall 号和参数，内核代为执行 | 操作系统 |
| Tool 的 JSON Schema | 声明工具名、参数、用途供 LLM 选择 | RPC IDL / OpenAPI 接口文档 | 后端 |
| 短期记忆（上下文窗口） | 本轮会话的临时上下文 | 内存 RAM：快，但易失、容量有限 | 操作系统 |
| 长期记忆（RAG / 向量库） | 跨会话的知识与偏好 | 磁盘 + 数据库索引：慢，但持久 | 操作系统 / 后端 |
| 上下文压缩 | 对话太长先摘要再继续 | 内存换页 / LRU 淘汰冷数据 | 操作系统 |
| Router / 意图分发 | 识别意图并转发给对应 Agent | 反向代理 / API 网关路由 | 后端 |
| Master-Slave 多 Agent | 主 Agent 派活，子 Agent 执行 | Boss-Worker 线程池 / 主从复制 | 操作系统 / 分布式 |
| Peer-to-Peer 协作 | Agent 平等协商、无中心节点 | P2P 网络 / Gossip 协议 | 分布式 |
| Agent 消息总线 | 请求-响应 / 广播 / 协商等结构化通信 | IPC（管道、信号）/ 消息队列 / Pub-Sub | 操作系统 / 后端 |
| 状态持久化 + Checkpoint | 保存现场，支持断点恢复 | 快照 / WAL / 游戏存档 | 操作系统 / 后端 |
| LangGraph 工作流编排 | 预定义节点图、确定性流转 | CI/CD Pipeline / 工作流引擎 | 后端 |
| Reflection 反思 | 产出后自我审查再修正 | Code Review / 编译-报错-修复循环 | 工程实践 |
| max-iterations 防死循环 | 限制最大循环轮数 | 看门狗 watchdog / 超时熔断 | 操作系统 |
| Human-in-the-loop | 关键操作暂停等人工确认 | sudo 授权 / 支付二次验证 | 操作系统 / 前端 |
| **LLM 决策本身** | 现场决定"下一步做什么" | **无经典对应物——唯一真正的新组件** | — |

**注意**：上表所有"派发/决策"位置，传统系统里是确定性代码（if/else、路由表），Agent 里换成了概率性的 LLM——同一个路由模式，Nginx 配置写死后十年不变，LLM Router 却可能幻觉出路由表里不存在的目标。因此 Agent 工程多出一套经典系统不需要的纪律：**评估（evals）、可观测性（Langfuse）、熔断与人工兜底**。模式是旧的，围绕不确定性建立的工程方法论是新的。
