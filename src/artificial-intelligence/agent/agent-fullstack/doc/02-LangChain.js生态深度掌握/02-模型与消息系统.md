...
// Google Gemini
const geminiModel = await initChatModel("google-genai:gemini-3.5-flash", {
  temperature: 0.3,
  timeout: 600_000,
});
```
...
当 Agent 组合多个工具调用和模型调用时，使用 `agent.stream` 获取完整状态快照：
...
> **注意**：旧版本中的 `streamEvents(..., { version: "v3" })` 在 LangChain.js v1 中已不推荐，官方统一使用 `agent.stream(..., { streamMode: "values" })`。
