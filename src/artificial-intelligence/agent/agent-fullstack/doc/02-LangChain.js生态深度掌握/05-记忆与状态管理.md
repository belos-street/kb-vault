...
// 清理策略示例：保留最近 N 个 Checkpoint
async function cleanupCheckpoints(threadId: string, keepLatest: number = 50) {
  const checkpoints = [];
  for await (const cp of checkpointer.list(threadId)) {
    checkpoints.push(cp);
  }
  // 按时间排序，删除旧的
  const toDelete = checkpoints.slice(keepLatest);
  for (const cp of toDelete) {
    await checkpointer.delete(threadId, cp.config.configurable.checkpoint_id);
  }
}
```

> 💡 **生产建议**：
> - 设置定时任务（Cron）定期清理过期 Checkpoint
