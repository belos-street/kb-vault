...
const agent = createAgent({
  ...
  middleware: [
    ...
    piiMiddleware("phone_number", { strategy: "mask", applyToInput: true }),
    piiMiddleware("credit_card", { strategy: "redact", applyToInput: true }),
    piiMiddleware("ssn", { strategy: "hash", applyToInput: true }),
  ],
});
```
...
**Q: piiMiddleware 的 block/redact/mask/hash 四种策略在实际业务中如何选择？在合规审计场景下哪种策略最合适？**
A: block（检测到即阻止调用）适合绝对不允许 PII 进入模型的场景；redact（替换为 [REDACTED]）适合不需要原始数据的场景，简单明了；mask（如 "138****1234"）适合需要保留部分信息用于确认的场景，但不适合严格合规；hash（替换为不可逆哈希）适合需要唯一标识但不能反推原文的场景（如日志关联）。合规审计推荐 redact + 审计日志记录原始数据的访问时间、访问者、原因，这样既保护了用户隐私，又保留了追溯能力。注意 piiMiddleware 按 PII 类型逐个实例化（`piiMiddleware(piiType, options)`），多个类型叠加多个中间件。
...
**Q: 自定义 Middleware 中，Hook 的返回值有哪些语义？如何实现"提前终止 Agent 执行"？**
A: 三种情况：1）不返回或返回 null → pass-through，不修改 State 也**不会**阻塞执行（常见误区是把 null 当成"拦截"）；2）返回 Partial<State> → 合并到当前 State；3）提前终止必须用 `jumpTo: "end"`（还可跳转到其他 Hook 节点），且该 Hook 必须以对象形式声明 `canJumpTo: ["end"]`，跳转目标受此声明约束。实际用途：安全检查不通过时直接结束；Token 预算耗尽时返回提示并终止；鉴权失败时短路。注意：要丢弃/替换模型输出应在 wrapModelCall 中处理，而不是依赖返回值语义。
