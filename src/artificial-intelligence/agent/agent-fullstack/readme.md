...
| 类型系统         | TypeScript                                         |
| Schema 校验    | Zod                                                |
| LLM          | Claude、GPT、GLM、DeepSeek、Qwen                       |
| Embedding 模型 | BGE-M3、Qwen3-Embedding                             |
| Agent 框架     | LangChain.js、LangGraph.js、DeepAgents、Vercel AI SDK |
| Agent 协议     | MCP（Model Context Protocol）、A2A                    |
| 向量数据库        | Milvus、Qdrant                                      |
| 搜索引擎         | ElasticSearch                                      |
| 知识图谱         | Neo4j                                              |
| 关系数据库        | PostgreSQL                                         |
...
| 可观测性         | LangSmith、Langfuse、Prometheus、Grafana              |
...
- 模型选型与成本优化策略
- **模型选型决策树**（2026年9月）：
  - 复杂推理任务 → GPT-5.6 Sol（深度思考模式）/ Claude Opus 5
  - 代码生成与 Agent 任务 → Claude Opus 5（编码双榜领先）/ Claude Sonnet 5（Agent 增强、性价比最优）
  - 多模态任务 → GPT-5.6 系列（图文视频）/ Gemini 3.8 Flash（原生多模态）
  - 长文档分析 → Claude Opus 5（100万 token）/ GPT-5.6（105万 token）
  - 成本敏感场景 → GLM-5.3 Flash（MIT 开源权重）/ DeepSeek V4-Flash / Kimi K3
  - 本地/私有化部署 → Qwen3.8 Max / GLM-5.3 / DeepSeek V4（开源权重）
...
- Hono 框架快速入门（对比 Fastify/Express：多运行时边缘部署、SSE 流式、RPC 端到端类型安全、Bun 原生支持最佳）
- 类型安全的 Agent 工具链设计
...
- 代码解释器（Code Interpreter）与程序化工具调用（PTC）
- Harness Profiles：面向开源模型（Qwen、DeepSeek、Kimi）的 harness 适配
- v0.7 精简 harness：移除默认 system prompt、todoListMiddleware 改为 opt-in（breaking changes）
- 可插拔文件系统后端
- 声明式文件权限控制
- 技能系统（Skills）与可复用工作流
...
### 4.1 Milvus / Qdrant 向量数据库

- Milvus 架构与核心概念
- Collection 设计与索引优化
- 向量相似度搜索算法
- 多向量字段与混合检索
- Milvus vs Qdrant 选型对比（分布式大规模 vs 轻量单机）
- **向量数据库选型决策树**：
...
- 目标分解与任务调度
- 自我反思与改进
- 浏览器自动化与 Computer Use（Playwright MCP、browser-use、E2B 沙箱）

### 9.4 Agent 生态与标准

- Agent 互操作性标准（MCP、A2A）
- 开源 Agent 框架对比
- 社区贡献与技术影响力
