...
abstract class ReActAgent {
  ...
  async run(task: string): Promise<string> { /* 固定循环 */ }
}
...
| 可测试性 | 便于 mock 子类 | 依赖注入要额外设计（见 review #2） |
...
export const getWeatherSchema = z.object({
  city: z.string().min(1, '城市名不能为空').describe('城市名称（支持中文名、英文名或别名...）')
})
...
**练习**：把教程 1.3 的 `SlidingWindowMemory`（窗口 = 10）搬进 `cli.ts`，history 超限时丢最旧的 user/assistant 对——防止长会话 token 爆炸（教程 1.1 成本意识的落地）。
...
| Bun 内置测试（1.4） | `bun:test` + `bun test`，27 个用例 |
...
**提示**：把 Step 2/3 的逻辑抽成 `executeToolCalls(messages, tools)` 递归函数；用 `re-act.ts` 现有的 `onStep` 透传事件。

**预期效果**：构造"查完北京天气后 LLM 又想查上海"的场景，CLI 能连续打印多组 🔧 📡。

### 练习 2：history 裁剪（短期记忆压缩）

**要求**：给 `cli.ts` 的 history 加滑动窗口（最多保留最近 5 轮 user/assistant），并解释为什么 tool 中间消息不应进 history。
