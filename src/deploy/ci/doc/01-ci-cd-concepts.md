...
| **生命周期** | GitHub 默认保留 90 天（GitLab 默认 30 天，用 `expire_in` 配置） | 短期保留，7 天无访问自动清除（GitHub Actions） |
| **跨 Pipeline** | 可以跨 Pipeline 下载和使用 | 作用域受限：GHA 默认分支的缓存所有分支可用，PR 分支只能用基分支的缓存 |
...
## 5. 架构对比：手工发布 vs 脚本化 vs CI/CD

| 维度 | 手工发布 | 脚本化部署 | CI/CD 平台 |
|------|---------|-----------|-----------|
| **触发方式** | 人工执行 | 人工执行脚本 | push / PR / 定时自动触发 |
| **一致性** | 依赖个人经验，易漏步骤 | 脚本固定，但环境漂移难发现 | 幂等构建 + 锁文件 + 干净 Runner |
| **反馈速度** | 上线后才发现问题 | 构建后才发现 | 秒级 Lint 反馈，问题在引入时暴露 |
| **审计追踪** | 聊天记录 / 口头交接 | 无运行记录 | 每次运行有完整日志与制品版本 |
| **回滚能力** | 靠记忆 | 靠脚本覆盖度 | 不可变制品 + 版本化回滚 |

> 面试中「为什么要用 CI/CD」类问题，本质是让你讲清这四列的差距。
...
- **要求**：为你当前负责的项目画一张从提交到部署的 Pipeline 图，标注每个 Stage 的预计耗时与失败策略
- **提示**：先用 §3.1「从快到慢」原则排 Stage 顺序，再检查哪些 Job 可以并行
- **预期效果**：一张 Mermaid 流程图，且能口头说明「为什么这个 Stage 排在那个前面」

### 练习 2：找出门禁反模式

- **要求**：审查项目现有 CI 配置，找出至少一个「死门禁」或缺失的门禁
- **提示**：对照 §4.2 门禁类型表逐项检查；注意 §4.3 的反面案例
- **预期效果**：一份改造清单——哪些门禁要加、哪些阈值要降、哪些改 Warning
