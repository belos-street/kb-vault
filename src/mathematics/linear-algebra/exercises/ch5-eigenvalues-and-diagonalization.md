...
**T1（解答）**：设 $A = \begin{pmatrix} 3 & 2 & -1 \\ 2 & 6 & -2 \\ -1 & -2 & 3 \end{pmatrix}$ ，求正交矩阵 $Q$ ，使 $Q^TAQ$ 为对角阵。
...
- $\lambda = 1$ （二重）： $(E - A)x = 0$ 即 $x_1 - x_3 = 0$ （ $x_2$ 任意），基础解系含两个向量 $\xi_1 = (1, 0, 1)^T$ ， $\xi_2 = (0, 1, 0)^T$ ；
- $\lambda = -1$ ： $(-E - A)x = 0$ 即 $x_1 + x_3 = 0$ 且 $x_2 = 0$ ，基础解系 $\xi_3 = (1, 0, -1)^T$ 。

重根 $\lambda = 1$ 满足 $3 - r(E - A) = 2 = k$ ，且 $A$ 是实对称矩阵（必可正交对角化），故 $A$ **可对角化**： $P = (\xi_1, \xi_2, \xi_3) = \begin{pmatrix} 1 & 0 & 1 \\ 0 & 1 & 0 \\ 1 & 0 & -1 \end{pmatrix}$ ， $\Lambda = \mathrm{diag}(1, 1, -1)$ （列与对角元对应； $\xi_2 = (0, 1, 0)^T$ 正是属于 $\lambda = 1$ 的另一个无关特征向量， $P$ 的每一列都是特征向量，无需补无关的非特征向量列）。

验算： $A\xi_1 = (1, 0, 1)^T = \xi_1$ ✓， $A\xi_2 = (0, 1, 0)^T = \xi_2$ ✓， $A\xi_3 = (-1, 0, -1)^T = -\xi_3$ ✓。
...
验算：该矩阵无行和结构，直接代回验证： $A\xi_2 = (0, 4, 4)^T = 4\xi_2$ ✓， $A\xi_3 = (0, 2, -2)^T = 2\xi_3$ ✓。
...
### T1
特征多项式按第一行展开（中间步： $M_{12} = -2(\lambda - 3) - 2 = -2\lambda + 4$ ， $M_{13} = -4 - (\lambda - 6) = 2 - \lambda$ ）：

$$|\lambda E - A| = (\lambda - 3)(\lambda^2 - 9\lambda + 14) + 2(-2\lambda + 4) + (2 - \lambda) = (\lambda - 2)\left[(\lambda - 3)(\lambda - 7) - 5\right] = (\lambda - 2)^2(\lambda - 8)$$

（自检： $\operatorname{tr}(A) = 12 = 2 + 2 + 8$ ✓； $|A| = 3(18 - 4) - 2(6 - 2) + (-1)(-4 + 6) = 42 - 8 - 2 = 32 = 2 \times 2 \times 8$ ✓；二阶主子式之和 $14 + 8 + 14 = 36 = 2 \times 2 + 2 \times 8 + 2 \times 8$ ✓。）

**求特征向量**：

- $\lambda = 2$ （二重）： $2E - A$ 的三行成比例， $r(2E - A) = 1$ ，只剩方程 $x_1 + 2x_2 - x_3 = 0$ ，基础解系取 $\xi_1 = (0, 1, 2)^T$ ， $\xi_2 = (1, 1, 3)^T$ 。内积 $\xi_1^T\xi_2 = 0 + 1 + 6 = 7 \ne 0$ ，**必须施密特**：

$$\beta_2 = \xi_2 - \frac{\xi_2^T\xi_1}{\xi_1^T\xi_1}\xi_1 = (1, 1, 3)^T - \frac{7}{5}(0, 1, 2)^T = \left(1, -\frac{2}{5}, \frac{1}{5}\right)^T \xrightarrow{\text{取整}} (5, -2, 1)^T$$

（核对： $5 - 4 - 1 = 0$ 仍在平面 $x_1 + 2x_2 - x_3 = 0$ 内 ✓，与 $\xi_1$ 的内积 $0 - 2 + 2 = 0$ ✓。）

- $\lambda = 8$ ： $(8E - A)x = 0$ 由第二行得 $x_2 = x_1 - x_3$ ，代入第一行得 $3(x_1 + x_3) = 0$ ，即 $x_3 = -x_1$ ，从而 $x_2 = 2x_1$ ， $\xi_3 = (1, 2, -1)^T$ （与 $\xi_1$ 、 $\beta_2$ 自动正交 ✓）。

**全部单位化**：

$$\eta_1 = \frac{1}{\sqrt{5}}(0, 1, 2)^T, \qquad \eta_2 = \frac{1}{\sqrt{30}}(5, -2, 1)^T, \qquad \eta_3 = \frac{1}{\sqrt{6}}(1, 2, -1)^T$$

$$Q = \begin{pmatrix} 0 & \frac{5}{\sqrt{30}} & \frac{1}{\sqrt{6}} \\ \frac{1}{\sqrt{5}} & -\frac{2}{\sqrt{30}} & \frac{2}{\sqrt{6}} \\ \frac{2}{\sqrt{5}} & \frac{1}{\sqrt{30}} & -\frac{1}{\sqrt{6}} \end{pmatrix}, \qquad Q^TAQ = \begin{pmatrix} 2 & & \\ & 2 & \\ & & 8 \end{pmatrix}$$

验算（特征向量逐个代回 $A\xi = \lambda\xi$ ）： $A(0, 1, 2)^T = (0, 2, 4)^T = 2\xi_1$ ✓， $A(1, 1, 3)^T = (2, 2, 6)^T = 2\xi_2$ ✓， $A(5, -2, 1)^T = (10, -4, 2)^T = 2\beta_2$ ✓， $A(1, 2, -1)^T = (8, 16, -8)^T = 8\xi_3$ ✓。 $Q^TQ = E$ ：三列模长平方 $\dfrac{0 + 1 + 4}{5} = 1$ ， $\dfrac{25 + 4 + 1}{30} = 1$ ， $\dfrac{1 + 4 + 1}{6} = 1$ ；两两内积 $\dfrac{0 - 2 + 2}{\sqrt{150}} = 0$ ， $\dfrac{0 + 2 - 2}{\sqrt{30}} = 0$ ， $\dfrac{5 - 4 - 1}{\sqrt{180}} = 0$ ✓。
