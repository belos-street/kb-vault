...
> 数列极限是数二大纲明确列出的考点——"极限存在的两个准则：单调有界准则和夹逼准则"，真题中常以大题或填空出现。函数极限的工具（等价无穷小、洛必达、泰勒展开）对离散的数列**不能直接使用**（数列没有导数），必须换用下面两个准则。

### 数列极限与函数极限的关系

若 $\lim_{x \to +\infty} f(x) = A$ ，把 $f(x)$ 限制在正整数点上取值，就得到 $\lim_{n \to \infty} f(n) = A$ 。

> **反之不成立**：数列极限存在推不出对应的函数极限存在（数列只是"采样点"）。但求递推数列极限时，有时可以借助对应的函数极限辅助分析。

### 准则一：夹逼准则

设数列 $\{a_n\}$ 、 $\{b_n\}$ 、 $\{c_n\}$ 满足：从某项起 $b_n \le a_n \le c_n$ ，且
...
**适用场景**：$n$ 项和的极限——每一项都在变，无法逐项计算，就整体放缩。

**例 14**：求 $\lim\limits_{n \to \infty}\left(\dfrac{1}{\sqrt{n^2+1}} + \dfrac{1}{\sqrt{n^2+2}} + \cdots + \dfrac{1}{\sqrt{n^2+n}}\right)$ 。

**解**：共 $n$ 项，每项都介于 $\dfrac{1}{\sqrt{n^2+n}}$ 与 $\dfrac{1}{\sqrt{n^2+1}}$ 之间，所以：

$$
\frac{n}{\sqrt{n^2+n}} \le \sum_{i=1}^{n}\frac{1}{\sqrt{n^2+i}} \le \frac{n}{\sqrt{n^2+1}}
$$

两端的极限都为 $1$ ，由夹逼准则，原极限 $= 1$ 。

### 准则二：单调有界准则

**单调递增且有上界（或单调递减且有下界）的数列必有极限。**

**适用场景**：递推数列 $a_{n+1} = f(a_n)$ ——先用数学归纳法证明"单调 + 有界"（即收敛），再对递推式两边取极限、解方程求出极限值。

**例 15**：设 $a_1 = \sqrt{2}$ ， $a_{n+1} = \sqrt{2 + a_n}$ ，证明 $\{a_n\}$ 收敛，并求极限。

**证明**（有界）：用归纳法证 $0 < a_n < 2$ 。 $a_1 = \sqrt{2} < 2$ ；若 $a_n < 2$ ，则 $a_{n+1} = \sqrt{2 + a_n} < \sqrt{4} = 2$ 。

**证明**（单调）： $a_{n+1}^2 - a_n^2 = (2 + a_n) - a_n^2 = (2 - a_n)(1 + a_n) > 0$ ，故 $a_{n+1} > a_n$ ，数列单调递增。

由单调有界准则， $\{a_n\}$ 收敛。设 $\lim\limits_{n \to \infty} a_n = A$ ，对递推式两边取极限：

$$
A = \sqrt{2 + A} \implies A^2 - A - 2 = 0 \implies A = 2 \text{（负根舍去）}
$$

### 与定积分定义的联系（预告）

另一类 $n$ 项和极限 $\lim\limits_{n \to \infty}\dfrac{1}{n}\displaystyle\sum_{i=1}^{n} f\!\left(\dfrac{i}{n}\right)$ 要用**定积分定义**处理（凑成黎曼和），详见第 3 章 2.1 节。

**判断标准**：每项能写成 $\dfrac{1}{n}f\!\left(\dfrac{i}{n}\right)$ 的形式 → 定积分定义；否则（如含 $\sqrt{n^2+i}$ ）→ 夹逼放缩。这两类 $n$ 项和极限是数二的固定考点。
...
> **要求**：不看解答独立完成，每题限时 5-8 分钟。**预期效果**：4 题全对可进入第 2 章；错 2 题以上，回到对应小节重做例题。

**练习 1**：求 $\lim\limits_{x \to 0}\dfrac{\tan x - x}{x^3}$ 。

- 💡 提示： $\tan x \sim x$ 在这里是加减相消场景，必须升级为泰勒展开 $\tan x = x + \dfrac{x^3}{3} + o(x^3)$ （可由 $\sin x$ 与 $\cos x$ 的展开推出）。
- ✅ 参考答案： $\dfrac{1}{3}$ 。

**练习 2**：求 $\lim\limits_{n \to \infty}\left(\dfrac{1}{n^2+1} + \dfrac{2}{n^2+2} + \cdots + \dfrac{n}{n^2+n}\right)$ 。

- 💡 提示：分子为 $1 + 2 + \cdots + n = \dfrac{n(n+1)}{2}$ ，对分母用 $n^2+1 \le n^2+i \le n^2+n$ 夹逼。
- ✅ 参考答案： $\dfrac{1}{2}$ （上下界均为 $\dfrac{n(n+1)/2}{n^2+n} \to \dfrac{1}{2}$ 与 $\dfrac{n(n+1)/2}{n^2+1} \to \dfrac{1}{2}$ ）。

**练习 3**：讨论 $f(x) = \dfrac{\sin x}{x}$ 的间断点 $x = 0$ 的类型。

- 💡 提示：算极限、看 $f(0)$ 是否有定义，对照第一类/第二类间断点的定义。
- ✅ 参考答案： $\lim\limits_{x \to 0}\dfrac{\sin x}{x} = 1$ 存在但 $f(0)$ 无定义，是**可去间断点**（补充定义 $f(0)=1$ 即连续）。

**练习 4**：证明方程 $x - \cos x = 0$ 在 $\left(0, \dfrac{\pi}{2}\right)$ 内至少有一个实根。

- 💡 提示：零点定理，检查端点函数值是否异号。
- ✅ 参考答案：令 $f(x) = x - \cos x$ ，在 $\left[0, \dfrac{\pi}{2}\right]$ 上连续， $f(0) = -1 < 0$ ， $f\!\left(\dfrac{\pi}{2}\right) = \dfrac{\pi}{2} > 0$ ，由零点定理即得。
