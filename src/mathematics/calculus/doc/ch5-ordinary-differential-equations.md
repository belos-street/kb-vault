...
**例 15**（几何应用）：设曲线 $y = y(x)$ （ $x > 0$ ）过点 $(1, 0)$ ，且曲线上任一点 $(x, y)$ 处的切线在 $y$ 轴上的截距等于 $\frac{y}{3}$ 。求该曲线方程。
...
$$\ln\lvert y\rvert = \frac{2}{3}\ln x + \ln\lvert C_1\rvert \quad (x > 0)$$

$$y = Cx^{2/3}$$

代入初值条件 $y(1) = 0$ ： $0 = C \cdot 1$ ，得 $C = 0$ 。

所以 $y = 0$ ？但这只是平凡解。让我们重新检查——分离变量时除以了 $y$ ，需要检查 $y = 0$ 是否是解。代入原方程： $0 = 0$ ，确实是解，且满足 $y(1) = 0$ 。

但 $y = 0$ 是唯一的解吗？通解 $y = Cx^{2/3}$ 中， $y(1) = C = 0$ ，确实只有 $C = 0$ 。

$$y = 0$$

> 这道题的结果虽然简单，但建模过程（切线截距条件的翻译）是典型的考试套路。
