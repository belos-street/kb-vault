...
**例 13**（ $\lambda$ 是单特征根）：求 $y'' - 2y' + y = e^x$ 的通解。
...
由 $y(1) = 0$：$0 = -\frac{1}{3} + C$，故 $C = \frac{1}{3}$。

$$\boxed{y = -\frac{x^2}{3} + \frac{1}{3x}}$$

**关键**：切线截距条件的翻译是本题核心：$y$ 轴截距 = $y - xy'$。翻译后得到一阶线性方程，直接套公式。注意 $x > 0$ 时 $\ln|x| = \ln x$，积分因子计算更简洁。
...
> 微分方程常与定积分应用、变限积分联合出题。以下两道题模拟真实考场的跨章组合。
...
$$(e^x y)' = e^x \cdot e^x = e^{2x}$$

$$e^x y = \frac{1}{2}e^{2x} + C$$

$$y = \frac{1}{2}e^x + Ce^{-x}$$

由 $y(0) = 0$：$0 = \frac{1}{2} + C$，故 $C = -\frac{1}{2}$。

$$y = \frac{1}{2}e^x - \frac{1}{2}e^{-x} = \sinh x$$

**第二步：求旋转体体积。**

$$V = \pi\int_0^1 y^2\,dx = \pi\int_0^1 \left(\frac{e^x - e^{-x}}{2}\right)^2 dx = \frac{\pi}{4}\int_0^1 (e^{2x} - 2 + e^{-2x})\,dx$$

$$= \frac{\pi}{4}\left[\frac{e^{2x}}{2} - 2x - \frac{e^{-2x}}{2}\right]_0^1 = \frac{\pi}{4}\left(\frac{e^2}{2} - 2 - \frac{e^{-2}}{2} - \frac{1}{2} + 0 + \frac{1}{2}\right)$$

$$= \frac{\pi}{4}\left(\frac{e^2 - e^{-2}}{2} - 2\right) = \frac{\pi}{4}(\sinh 2 - 2)$$

**关键**：本题是"微分方程 + 旋转体体积"的经典组合。解方程后得到 $y = \sinh x$，平方积分时展开为 $e^{2x} - 2 + e^{-2x}$ 三项分别积分。
