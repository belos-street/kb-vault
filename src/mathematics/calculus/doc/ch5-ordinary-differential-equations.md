...
有些高阶方程不直接包含 $y$ （或不直接包含 $x$ ），可以通过换元降低阶数，化为一阶方程来解。数二主要考以下三种类型：

| 类型 | 形式 | 特征 |
|------|------|------|
| **最简型** | $y^{(n)} = f(x)$ | 只出现最高阶导数，直接积分 $n$ 次 |
| **缺 $y$ 型** | $y'' = f(x, y')$ | 方程中不出现 $y$ 本身 |
| **缺 $x$ 型** | $y'' = f(y, y')$ | 方程中不出现 $x$ 本身 |

其中**最简型**最简单：方程中只出现最高阶导数，两边直接积分 $n$ 次即可，每积分一次出现一个任意常数。例如 $y'' = e^{2x}$ ，积分两次得 $y = \dfrac{1}{4}e^{2x} + C_1x + C_2$ 。这类题计算量小，常作为大题的第一问或填空题出现。
...
| 最简型 | $y^{(n)} = f(x)$，只含最高阶导数 | 连续积分 $n$ 次 | 每积分一次出现一个任意常数 |
...
> **要求**：独立完成，每题限时 5-8 分钟；解完把解代回原方程验证。**预期效果**：看到方程 10 秒内说出类型，二阶常系数方程"齐次通解 + 特解"两步不乱。

**练习 1**：求 $y' + y = e^{-x}$ 的通解。

- 💡 提示：一阶线性， $p(x) = 1$ ， $q(x) = e^{-x}$ 。
- ✅ 参考答案： $y = e^{-x}\left(\displaystyle\int e^{-x} \cdot e^x\,dx + C\right) = (x + C)e^{-x}$ 。

**练习 2**：求 $y'' + 3y' + 2y = 0$ 的通解。

- 💡 提示：特征方程 $r^2 + 3r + 2 = 0$ ，因式分解 $(r+1)(r+2) = 0$ 。
- ✅ 参考答案： $y = C_1 e^{-x} + C_2 e^{-2x}$ 。

**练习 3**：求 $y'' - y' = x$ 的通解。

- 💡 提示：齐次特征根 $r = 0, 1$ ；$f(x) = x = x \cdot e^{0 \cdot x}$ ，$\lambda = 0$ 恰是**单特征根**，特解要乘 $x$ （设 $y^* = x(ax + b)$）。
- ✅ 参考答案： $y = C_1 + C_2 e^x - \dfrac{x^2}{2} - x$ 。

**练习 4**：求 $y''' = e^{2x}$ 的通解。

- 💡 提示：最简型 $y^{(n)} = f(x)$ ，直接积分三次，每积分一次出现一个常数。
- ✅ 参考答案： $y = \dfrac{1}{8}e^{2x} + \dfrac{C_1}{2}x^2 + C_2 x + C_3$ 。
