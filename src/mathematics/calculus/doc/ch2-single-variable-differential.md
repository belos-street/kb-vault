...
设 $y = f(x)$ 严格单调、可导且 $f'(x) \neq 0$ ，其反函数 $x = f^{-1}(y)$ 可导，且：

$$
\left(f^{-1}\right)'(y) = \frac{1}{f'(x)} \quad \left(\text{即 } \frac{dx}{dy} = \frac{1}{\;dy/dx\;}\right)
$$

**例**：由 $x = \sin y$ （ $y \in \left(-\frac{\pi}{2}, \frac{\pi}{2}\right)$ ）推 $\arcsin$ 的导数： $\dfrac{dx}{dy} = \dfrac{1}{\cos y} = \dfrac{1}{\sqrt{1 - \sin^2 y}} = \dfrac{1}{\sqrt{1 - x^2}}$ ，这正是基本求导公式表中 $\arcsin$ 一行的来源。
...
| $f'(\xi)\cos\xi + f(\xi)\sin\xi = 0$ | $F(x) = \dfrac{f(x)}{\cos x}$ | 商的求导 $\left(\dfrac{f}{\cos x}\right)' = \dfrac{f'\cos x + f\sin x}{\cos^2 x}$ |
...
**曲率圆**：以曲率半径 $R$ 为半径、圆心位于曲线该点**凹侧法线**上的圆，称为曲线在该点的**曲率圆**（密切圆）。曲率圆与曲线在该点有相同的切线、凹向与曲率。大纲将"曲率圆与曲率半径"并列列出，选择题可能直接考概念。
...
flowchart TD
    A["题目类型？"] --> B["求导/求微分"]
    A --> C["求极限（0/0 型，含 e^x, sin x, ln 等）"]
    A --> D["证明等式（含 f'(ξ)）"]
    A --> E["证明不等式"]
    A --> F["求极值/最值"]
    A --> G["求渐近线"]
    ...
    B --> B1["基本公式 + 链式法则<br/>（注意隐函数、参数方程）"]

    C --> C1["简单 → 等价无穷小"]
    C --> C2["相消 → 泰勒展开<br/>（展到第一个非零项）"]

    D --> D1["f'(ξ) = 常数 → 拉格朗日"]
    D --> D2["f'(ξ) = 0 → 罗尔"]
    D --> D3["含 f'(ξ) 和 f(ξ) → 构造辅助函数 + 罗尔"]
    D --> D4["含两个函数 → 柯西"]

    E --> E1["构造 F(x) = 左 - 右，证单调性"]
    E --> E2["拉格朗日中值定理"]
    E --> E3["泰勒展开（取余项符号）"]

    F --> F1["f'(x) = 0 找驻点 + 比较"]
    G --> G1["三种分别求"]
    H --> H1["定义域 → 奇偶 → f' → f'' → 渐近线 → 列表 → 画图"]
```
...
> **要求**：独立完成，每题限时 8-10 分钟。**预期效果**：求导公式"默写级"熟练；中值定理证明能独立完成"构造辅助函数 → 验证罗尔条件"两步。

**练习 1**：用反函数求导法则从 $x = \tan y$ 推出 $y = \arctan x$ 的导数（不要直接背公式）。

- 💡 提示： $\left(f^{-1}\right)' = \dfrac{1}{f'}$ ，再用 $\cos^2 y = \dfrac{1}{1 + \tan^2 y}$ 。
- ✅ 参考答案： $(\arctan x)' = \dfrac{1}{1 + x^2}$ 。

**练习 2**：设 $y = y(x)$ 由方程 $x^3 + y^3 - 3xy = 0$ 确定，求 $y'$ 。

- 💡 提示：两边对 $x$ 求导，含 $y$ 的项都要乘 $y'$ 。
- ✅ 参考答案： $y' = \dfrac{y - x^2}{y^2 - x}$ 。

**练习 3**：证明：当 $x > 0$ 且 $x \neq e$ 时， $\dfrac{\ln x}{x} < \dfrac{1}{e}$ 。

- 💡 提示：令 $f(x) = \dfrac{\ln x}{x}$ ，求最大值——驻点 $x = e$ ，由 $f'(x) = \dfrac{1 - \ln x}{x^2}$ 判定单调性。
- ✅ 参考答案： $f(x)$ 在 $x = e$ 处取唯一极大值（即最大值）$\dfrac{1}{e}$ ，故其余点处严格小于。

**练习 4**：设 $f(x)$ 在 $[0, 1]$ 上连续，在 $(0, 1)$ 内可导，且 $f(0) = f(1) = 0$ 。证明存在 $\xi \in (0, 1)$ 使 $(1 + \xi^2)f'(\xi) = 2\xi f(\xi)$ 。

- 💡 提示：对照 §2.7 模式表，结论即 $\dfrac{(1+x^2)f'(x) - 2xf(x)}{(1+x^2)^2} = 0$ 的分子为零，构造 $F(x) = \dfrac{f(x)}{1 + x^2}$ 。
- ✅ 参考答案： $F(0) = f(0) = 0$ ， $F(1) = \dfrac{f(1)}{2} = 0$ ，对 $F$ 用罗尔定理即得。
