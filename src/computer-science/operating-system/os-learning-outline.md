...
| 项目 | 说明 |
|------|------|
| **考试大纲** | 408 计算机学科专业基础 — 操作系统部分 |
| **分值占比** | 约 35 分 / 150 分（选择题 + 综合应用题） |
...
| 文件管理 | 文件物理结构、磁盘调度 | 选择题为主 | ⭐⭐⭐ |
| I/O 管理 | I/O 控制方式、缓冲计算 | 选择题为主 | ⭐⭐⭐ |
...
| **第 3 章** | 处理机调度 | 能计算 FCFS/SJF/RR/优先级/多级反馈队列的周转时间 |
| **第 4 章** | 进程同步与互斥 | 能用 PV 操作解决生产者-消费者、读者-写者、哲学家进餐 |
| **第 5 章** | 死锁 | 掌握银行家算法、死锁检测与解除 |
| **第 6 章** | 内存管理基础 | 理解连续分配、分页/分段地址变换 |
| **第 7 章** | 虚拟内存 | 掌握请求分页、页面置换算法（OPT/FIFO/LRU/Clock）、工作集 |
...
├── doc/
│   ├── 01-overview.md                        # 操作系统概述
│   ├── 02-process-and-thread.md              # 进程与线程
│   ├── 03-cpu-scheduling.md                  # 处理机调度
│   ├── 04-synchronization.md                 # 进程同步与互斥
│   ├── 05-deadlock.md                        # 死锁
│   ├── 06-memory-management.md               # 内存管理基础
│   ├── 07-virtual-memory.md                  # 虚拟内存
│   ├── 08-file-management.md                 # 文件管理
│   └── 09-io-management.md                   # I/O 管理
└── assets/                                   # 图表、示意图
```
...
| 磁盘调度计算 | 第 8 章 | 磁道移动总数、平均寻道时间 |
| 缓冲时间计算 | 第 9 章 | 单缓冲： $T_{total} = T + \max(C, M)$；双缓冲： $T_{total} = \max(T, C, M)$（详见 doc） |
...
- [ ] 能用 OPT/FIFO/LRU/Clock 模拟页面置换并计算缺页率
- [ ] 能计算索引文件的寻址范围
...
5. **文件与 I/O 偏记忆**：理解原理后记住关键结论即可，不需要大量计算
