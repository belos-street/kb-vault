...
- **重定位寄存器（基址寄存器）**：存放程序在内存中的起始物理地址
- 物理地址 $=$ 重定位寄存器（基址寄存器） $+$ 逻辑地址（相对偏移量），每次访存自动完成
...
$$
EAT = \max(t_{TLB}, t_{mem}) + t_{mem}
$$

TLB 与内存页表同时查询，并行查询完成后，**无论命中与否**，都要再访存一次访问目标单元；并行的收益是 TLB 缺失不再带来额外访存代价（缺失时间被页表访存覆盖），故 EAT 与命中率 $\lambda$ 无关。当 $t_{TLB} \le t_{mem}$ 时， $EAT = 2 t_{mem}$ 。
...
- **BF**（按容量递增选最小够用者）：80 KB → 分区 2（350~430，余 430/20）；130 KB → 此时空闲区为 150/20/200，分区 1 最小够用（100~230，余 230/20）；30 KB → 剩余空闲区为 20/20/200，仅分区 3 够用（600~630，余 630/170）。
...
| 130 KB | 600~730 | 100~230 | 100~230 | 600~730 |
| 30 KB | 180~210 | 600~630 | 680~710 | 730~760 |
...
**并行**：命中 $=$ 缺失 $= \max(20, 100) + 100 = 200\text{ ns}$ （并行完成地址变换后，还要再访存一次访问目标单元）。

$$
EAT = 200\text{ ns}
$$

并行时 EAT 与命中率无关；由于 $t_{TLB} \le t_{mem}$ ，EAT 恒等于 $2 \times t_{mem}$ 。
...
5. **TLB EAT**：串行 $EAT = \lambda(t_{TLB}+t_{mem}) + (1-\lambda)(t_{TLB}+2t_{mem})$ ；并行 $EAT = \max(t_{TLB}, t_{mem}) + t_{mem}$ ，与命中率无关
