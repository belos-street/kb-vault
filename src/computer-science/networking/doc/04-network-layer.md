...
**间接路由（即三角路由）**：发往移动主机的报文先发给其归属地址，由归属代理（家乡代理）截获后经隧道封装转发到外地代理，再交给移动主机；而移动主机的回复报文直接发给通信方，不必绕道归属代理。于是「通信方 → 归属代理 → 移动主机 → 通信方」的路径呈三角形，故间接路由又称三角路由。

**路由优化**：归属代理把移动主机的转交地址告知通信方后，通信方可直接把报文发给转交地址，减少经归属代理的绕路。
