...
> 这章是 408 计网最重要的一章，重要度 ⭐⭐⭐⭐⭐。**三次握手、四次挥手、拥塞控制**是选择题与综合题的双重高频考点，几乎每年必出一题；2016、2023 等年份的综合题都直接考 TCP 拥塞窗口推演。网络层解决的是**主机到主机**的通信，传输层在此之上解决**进程到进程**的通信，并为应用层提供 UDP（不可靠、尽力而为）和 TCP（可靠、面向连接）两种服务。本章建议投入 6-8 小时，重点是状态推演类题目的动手练习。
...
| 字段 | 长度 | 说明 |
|------|------|------|
| 源端口 | 2 字节 | 可全 0（不需要对方回复时） |
| 目的端口 | 2 字节 | 交付给哪个进程 |
| 长度 | 2 字节 | UDP 用户数据报的总长度（首部 + 数据），最小值 8 |
| 校验和 | 2 字节 | 检测差错；计算时加**伪首部** |
...
| 字段 | 长度 | 说明 |
|------|------|------|
| 源端口 / 目的端口 | 各 16 bit | 标识通信的两个应用进程 |
| 序号 seq | 32 bit | 本报文段**第一个数据字节**的序号 |
| 确认号 ack | 32 bit | **期望收到**的下一个字节的序号 |
| 数据偏移 | 4 bit | 首部长度，单位 32 bit（4 字节） |
| 保留 | 6 bit | 保留为 0 |
| 6 个标志位 | 各 1 bit | URG / ACK / PSH / RST / SYN / FIN |
| 窗口 | 16 bit | 接收窗口 rwnd，流量控制用 |
| 校验和 | 16 bit | 计算时也要加伪首部 |
| 紧急指针 | 16 bit | 配合 URG 指出紧急数据末尾位置 |
...
- RTT 样本通过 `新样本 = 实际测量值` 、 `RTT_s = (1-\alpha) RTT_s + \alpha \times 样本` 加权平滑
