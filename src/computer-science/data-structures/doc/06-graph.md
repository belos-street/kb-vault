...
MST 总权值 $= 1 + 2 + 3 + 4 + 5 = 15$ 。对比例 2 可以发现：两算法**选边次序不同**（Prim 按 $(v_1,v_3),(v_3,v_5),(v_5,v_2),(v_2,v_4),(v_4,v_6)$ 的顺序选入，Kruskal 按权值 $1,2,3,4,5$ 的升序选入），但最终**边集完全相同**，得到的是同一棵 MST。本例被选中的 5 条边权值 $1 \sim 5$ 互不相同，因此该图的 MST 唯一。若图中存在等权边，MST 才可能不唯一（但权值和仍然相同）。
