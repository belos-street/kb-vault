...
| **地址总线** | AB | 内存/IO 端口地址 | **单向**（CPU → 内存） | 寻址范围， $n$ 位可寻址 $2^n$ 个单元 |
| **控制总线** | CB | 控制信号、状态信号 | 每条线方向固定，整体**双向** | 控制功能的多少 |

> **关键区分**：数据总线是**双向**的（既能读也能写），地址总线是**单向**的（地址只能由 CPU 发出）。选择题常考"哪种总线是双向的"，答案是数据总线。控制总线中每一根线方向固定，但作为一个整体有入有出，所以说整体双向。
