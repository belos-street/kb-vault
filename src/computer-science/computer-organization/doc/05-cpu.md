...
**例 2**：单总线结构 CPU，执行指令 `ADD R1, [X]`（将内存地址 X 处的数据与 R1 相加，结果存回 R1）。列出执行阶段的各节拍数据流。
...
**例 3**：某 5 段流水线，每段耗时均为 2 ns。执行 100 条指令，求：（1）流水线执行时间；（2）加速比；（3）吞吐率；（4）效率。
...
**例 4**：在 5 段流水线中执行以下指令序列，假设采用转发机制：
...
- I2 依赖 I1 的 R1：I1 在第 4 周期完成 MEM，第 5 周期 WB；I2 名义在第 4 周期需要 EX → 暂停 2 周期
- I3 依赖 I2 的 R3：同理暂停 2 周期
- 共暂停 **4 个周期**

（2）有转发：
- I1 是 Load 指令，数据在 MEM 段末（第 4 周期末）才可用，而 I2 名义在第 4 周期进入 EX 就需要 R1 → 必须暂停 **1 个周期**（Load-Use 冒险），I2 的 EX 实际在第 5 周期执行
- I2 是 ALU 指令（ADD），结果在 EX 段末（第 5 周期末）才产生；I3 名义也在第 5 周期进入 EX，而 EX/EX 转发要求生产者比消费者早一拍进 EX，此时无法满足 → 再暂停 **1 个周期**，I3 的 EX 实际在第 6 周期执行
- 共暂停 **2 个周期**

> **考试技巧**：Load-Use 冒险是转发无法完全消除的，必须暂停 1 拍。非 Load 的 RAW 冒险（如 ADD 后跟 SUB），转发通常可以完全消除。但要注意：若前方 Load-Use 已引起暂停，后续 ALU 指令的 EX 可能与被推迟的前驱指令撞上同一周期，此时仍需再暂停 1 拍——转发不能完全消除连锁冒险。这是 408 综合题的经典考法。
...
**例 5**：5 段流水线中，条件分支指令在 EX 段末尾确定是否跳转。若分支成功，需要冲刷几条已取出的指令？若采用"总是预测不跳转"策略，分支成功时的惩罚是几个周期？
...
**例 6**：某微程序控制器采用水平型微指令，微指令字长 32 位，其中操作控制字段 24 位（直接编码），顺序控制字段 8 位。若共有 40 个微命令，问：（1）操作控制字段能否容纳所有微命令？（2）顺序控制字段能寻址多少条微指令？
...
> **微指令位数判定**：直接编码下每个微命令占 1 位；字段（间接）编码将互斥微命令分组，每组占 $\lceil \log_2(组内命令数) \rceil$ 位。这是选择题常考的细节。
...
1. 一条指令的执行过程被划分为取指、译码、执行、访存、写回 5 个阶段，这描述的是什么概念？指令周期、机器周期、时钟周期三者的大小关系如何？
2. 某 6 段流水线，每段耗时 2 ns，执行 50 条指令，流水线执行时间是多少？（忽略建立与排空）
3. Load-Use 数据冒险中，即使有转发机制也必须暂停 1 个周期，原因是什么？
4. 微程序控制器中，微指令地址的形成方式有哪几种？

> **答案**：1. 指令周期；指令周期 > 机器周期 > 时钟周期；2. $(6 + 50 - 1) \times 2 = 110$ ns；3. Load 的结果要到 MEM 段才可用，而后续指令在 EX 段就需要该数据；4. 增量计数（顺序 +1）、下地址字段、根据机器指令操作码形成、根据状态标志形成。
