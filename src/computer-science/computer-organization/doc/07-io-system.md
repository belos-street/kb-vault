...
```mermaid
graph LR
    A["程序查询：CPU 全程轮询"] --> B["中断：按字节, CPU 频繁介入"]
    B --> C["DMA：按数据块, CPU 基本不参与"]
    C --> D["通道：专用 I/O 处理器"]
```
...
**例 3**：某设备采用中断方式传送数据，每准备好 1 字节就向 CPU 发一次中断，中断响应和服务共耗 20 个 CPU 周期。CPU 主频 50 MHz，设备传输速率 10 KB/s。求每秒 I/O 中断消耗的 CPU 时间占比。若改用 DMA 方式，预处理需 100 条指令、后处理需 50 条指令，传送 1 KB 数据 CPU 只需介入两次，则消耗占比又是多少？

**解**：

（1）中断方式：

- CPU 每秒周期数 = $5 \times 10^7$ （主频 50 MHz，每个周期 0.02 μs）
- 设备每秒传送 $10\text{ KB/s} = 10 \times 1024 = 10240$ 字节，每字节中断一次，共 10240 次中断
- 中断总开销 = $10240 \times 20 = 204800$ 个周期
- 占 CPU 时间比例 = $204800 / (5 \times 10^7) \approx 0.41\%$

（2）DMA 方式：

- 传送 1 KB 数据，CPU 只介入两次：预处理（设置 DMAC 寄存器，100 条指令）+ 后处理（中断收尾，50 条指令）
- CPU 开销 = $100 + 50 = 150$ 个周期
- 占比 = $150 / (5 \times 10^7) = 3 \times 10^{-6} \approx 0.0003\%$

> 点评：中断方式的开销与字节数成正比（每字节都要打断一次），DMA 方式的开销只与"块"有关——传输速率越高、数据量越大，DMA 的优势越明显，这就是磁盘等高速大块传输设备采用 DMA 的原因。计算题套路固定：每秒 CPU 介入次数 × 每次周期数 ÷ CPU 每秒总周期数。

**例 4**：某磁盘转速 7200 r/min，平均寻道时间 8 ms，每道 1000 个扇区，每扇区 512 B，数据传输率 50 MB/s。求读取一个扇区的平均存取时间。

**解**：

磁盘存取时间 = 寻道时间 + 旋转延迟 + 传输时间：

- 寻道时间：8 ms（题目给定的平均寻道时间）
- 旋转延迟：转一圈时间 = $60 / 7200$ s $\approx 8.33$ ms，平均旋转延迟按半圈计 = $8.33 / 2 \approx 4.17$ ms
- 传输时间： $512 / (50 \times 10^6)$ s $\approx 0.01$ ms
- 平均存取时间 $\approx 8 + 4.17 + 0.01 \approx 12.18$ ms

> 点评：记住"存取时间 = 寻道 + 旋转延迟 + 传输"三段式。**平均旋转延迟 = 半圈时间**（转一圈时间 = 60 / 转速）。传输率按 1 MB = $10^6$ B 计；单扇区传输时间通常极小，但步骤不能省。
...
> 这条链路把第 7 章（I/O 硬件）和[操作系统](../../operating-system/os-learning-outline.md)（设备管理、中断处理）串了起来。408 的计组和 OS 经常在这里交叉出题：计组考"硬件怎么传"，OS 考"软件怎么管"。
...
>
> **与其他章节的关联**：中断响应条件中"当前指令执行完毕"对应[第 5 章（CPU）](05-cpu.md)的指令执行流程；DMA 的总线交互对应[第 3 章（存储系统）](03-memory-system.md)的总线结构；I/O 与 OS 的关联则是操作系统"设备管理"和"中断处理"章节的硬件基础。
...
1. 中断响应需要同时满足哪些条件？
2. 中断屏蔽字中某位为 1 代表什么含义？
3. DMA 传输结束后，DMAC 如何通知 CPU？
4. 中断嵌套中多个中断服务的返回次序遵循什么原则？

> **答案**：1. 有中断请求、CPU 处于开中断状态、当前指令已执行完毕（且该中断未被屏蔽）；2. 为 1 表示屏蔽该中断源（不可被打断），为 0 表示允许打断；3. 通过中断请求通知 CPU（DMA 只是传输过程中不需要 CPU 逐字节干预，结束仍需中断）；4. 后响应的先返回（先进后出，类似函数调用栈）。
