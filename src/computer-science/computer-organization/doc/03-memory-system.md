...
> 存储系统是计组两大核心之一（另一个是 CPU/流水线），重要度 ⭐⭐⭐⭐⭐。Cache 地址划分、命中率计算、主存扩展几乎每年必考，选择题和综合题都会出。这一章的套路非常固定：记住地址结构怎么切、容量怎么算、扩展怎么做，拿分就是水到渠成的事。整章建议投入约 3 小时。
...
> 408 选择题考法：给定行数（如 1024 行）和存取周期（如 0.5 μs），问集中刷新的"死时间"。
> 死时间 = 行数 × 存取周期（集中刷新逐行进行，每刷新一行都要占用一次存取周期，期间不能读写）。1024 行 × 0.5 μs = 512 μs。
...
**例 8**：虚拟地址 32 位，页大小 4 KB，页表项 4 B。若采用单级页表，页表占多大空间？为何必须采用多级页表？若每级页表恰好装入一个页面，需要几级页表？TLB 缺失时访问一次数据最多访存几次？

**解**：

- 页内偏移 = $\log_2(4\text{ KB}) = 12$ 位，虚拟页号 = $32 - 12 = 20$ 位
- 单级页表共 $2^{20}$ 个表项，总容量 = $2^{20} \times 4$ B = 4 MB，远超一个页面（4 KB），常驻内存代价太大 → 必须采用多级页表
- 每页可装 $4\text{ KB} / 4$ B = 1024 = $2^{10}$ 个表项，即一级页表用 10 位页号索引恰好占一页
- 20 位页号拆成 2 段（各 10 位）→ 需要**二级页表**
- TLB 缺失时最多访存：查一级页表 1 次 + 查二级页表 1 次 + 取数据 1 次 = **3 次**

> 点评：页表容量 = 表项数（ $= 2^{页号位数}$ ）× 表项大小。单级页表超过一页就必须分级；级数由"一页能索引多少页号位"决定，即 $\log_2(页大小 / 表项大小)$ 位一级。 $k$ 级页表 TLB 缺失时最多访存 $k + 1$ 次，别忘了最后取数据的那 1 次。
...
时间示意（4 模块低位交叉，设 $T = 4\tau$ ，每列表示一个 $\tau$ ）：

| 模块 | 1 | 2 | 3 | 4 | 5 | 6 | 7 |
|------|---|---|---|---|---|---|---|
| 模块 0 | 字 0 | 字 0 | 字 0 | 字 0 |  |  |  |
| 模块 1 |  | 字 1 | 字 1 | 字 1 | 字 1 |  |  |
| 模块 2 |  |  | 字 2 | 字 2 | 字 2 | 字 2 |  |
| 模块 3 |  |  |  | 字 3 | 字 3 | 字 3 | 字 3 |
| 总线送出 |  |  |  | 字 0 | 字 1 | 字 2 | 字 3 |

- 模块 0 先开始访问，之后每隔 $\tau$ 多启动一个模块
- 每个模块内部相邻两次访问间隔 $T$ （等恢复周期），但总线上每隔 $\tau$ 就有一个字送出
- 连续访问 $m$ 个字总时间 $T + (m-1)\tau$ ，等效总线周期 $= \tau$ （远小于 $T$ ）
...
1. Cache 的三种映射方式（直接、全相联、组相联）中，哪种没有行号/组号字段？
2. 直接映射下，主存块号为 $i$ 的块只能放入 Cache 的哪一行？
3. 某 Cache 采用 4 路组相联，共 128 行，块大小 32 B，主存地址 32 位、按字节编址。标记字段为多少位？
4. 二级页表且 TLB 缺失时，不考虑缺页，一次访存最多访问几次主存？

> **答案**：1. 全相联；2. 第 $i \bmod C$ 行（ $C$ 为 Cache 行数）；3. 22 位（组数 $= 128 / 4 = 32$ ，组号 5 位；块内地址 $= \log_2 32 = 5$ 位；标记 $= 32 - 5 - 5 = 22$ 位）；4. 3 次（查一级页表 + 查二级页表 + 取数据）。
