...
```js
function setupExport(stage, layerManager) {
    ...
    input.onchange = (e) => {
      ...
      reader.onload = (evt) => {
        try {
          ...
          // 销毁旧 Layer，保留 Stage 引用
          stage.find('Layer').forEach(l => l.destroy());
          layerManager.layers = [];

          const stageData = json.className === 'Stage'
            ? json
            : { attrs: {}, children: [] };

          // 逐 Layer 重建，逐节点创建并绑定事件
          ;(stageData.children || []).forEach(layerData => {
            if (layerData.className !== 'Layer') return;
            const layer = new Konva.Layer({
              name: layerData.attrs?.name || `图层 ${layerManager.layers.length + 1}`
            });
...
> **为什么不用 `Konva.Node.create(jsonString, 'container')`？**
>
> 这个 API 会创建一个新的 Stage 并替换容器 DOM，导致原 `stage` 引用失效，`state` 中的 `stage/layer/tr` 全部指向旧对象，后续操作全部失效。正确做法是：**保留原 Stage，只销毁/重建 Layer**。
