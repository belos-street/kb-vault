根据你"快速上手、面试能答、做出 3D 效果"的目标，这份大纲把 Three.js 学习拆解成 **7 个模块**，每天 1~2 小时，**约两周完成**。每个模块都对应到面试常问点，最后用一个**3D 产品展示器** Demo 串联全部知识。
...
### 模块 2：几何体与材质（1 天）
**面试可答**：几何体定义形状，材质定义外观，两者组合成 Mesh。
...
### 模块 5：模型加载（1 天）
**面试可答**：Three.js 支持 glTF、OBJ、FBX 等格式，glTF 是推荐格式。
...
### 模块 6：动画与特效（1 天）
**面试可答**：`requestAnimationFrame` 驱动动画，`GSAP` 可做复杂补间。

- `requestAnimationFrame` 渲染循环
- 对象动画：位移、旋转、缩放
- 粒子系统：`Points`、`BufferGeometry`
- 后处理：`EffectComposer`（可选）
- **练习**：创建旋转动画和粒子效果

---

### 模块 7：进阶技巧与实战整合（2 天）
**面试可答**：性能优化用实例化渲染、LOD，响应式需监听 resize。
...
| 时间段 | 内容 | 积累成果 |
|--------|------|----------|
| 第 1 天 | 模块 1+2 | Scene/Camera/Renderer、几何体材质 |
| 第 2-3 天 | 模块 3+4 | 光照阴影、相机控制 |
| 第 4 天 | 模块 5 | 模型加载 |
| 第 5 天 | 模块 6 | 动画与粒子 |
| 第 6-10 天 | 模块 7 + 产品展示器 | 优化、完整项目 |
