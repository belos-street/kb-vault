...
```
需要快速出原型？
├── 是 → Ant Design 或 Shadcn/ui
└── 否
    ├── 需要高度自定义 → Radix UI + Tailwind
    ├── 企业 B 端 → Ant Design 或 Arco Design
    └── 消费级产品 → Shadcn/ui 或 Chakra UI
```
...
│   └── electron-store
├── < 10,000 条
│   ├── 简单查询 → lowdb 或 better-sqlite3
│   └── 复杂查询 → better-sqlite3
└── > 10,000 条
    └── better-sqlite3（+ Drizzle ORM 可选）
```
...
| **ffi-napi** | 3k+ | 调用 C/C++ 动态库 | ✅ |
| **keytar** | 2k+ | 系统密钥链（安全存储密码） | ✅ |
| **robotjs** | 3k+ | 键鼠模拟（自动化测试） | ✅ |
...
你在做什么？
├── 状态管理
│   └── Zustand（首选）/ Redux Toolkit（大型项目）
├── UI 组件
│   └── Shadcn/ui（首选）/ Ant Design（企业级）
├── 代码编辑器
│   └── CodeMirror 6（轻量）/ Monaco（功能全）
├── 富文本编辑器
│   └── Tiptap（首选）
├── 数据存储
│   └── electron-store（配置）/ better-sqlite3（数据）
├── 打包分发
│   └── Electron Forge（新项目）/ Electron Builder（已有项目）
├── 自动更新
│   └── electron-updater + GitHub Releases
├── 国际化
│   └── i18next + react-i18next
└── 测试
    └── Playwright（E2E）+ Vitest（单元）
```
