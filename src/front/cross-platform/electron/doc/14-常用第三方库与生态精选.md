...
> 面试可答：Electron 技术栈选型口诀——状态管理 Zustand、UI 组件 Shadcn/ui（自定义）或 Ant Design（企业级）、数据存储 electron-store（配置）/ better-sqlite3（结构化数据）、打包 Electron Forge（新项目）或 Electron Builder（已有项目）、测试 Playwright（E2E）+ Vitest（单元）、国际化 i18next。
...
详见 [11 — 调试、构建与打包分发](./11-调试、构建与打包分发.md)。
