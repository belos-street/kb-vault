...
│  │  UI 渲染（HTML/CSS/JS）             │  │
│  │  +                                 │  │
│  │  系统能力（fs / child_process）     │  │
│  └────────────────────────────────────┘  │
│  问题：XSS 一来，直接读文件 + 执行命令    │
└──────────────────────────────────────────┘
```
...
```
主进程（1 个）
├── BrowserWindow A → 渲染进程 A（独立 Chromium 实例）
├── BrowserWindow B → 渲染进程 B（独立 Chromium 实例）
└── BrowserWindow C → 渲染进程 C（独立 Chromium 实例）
```
...
```
❌ 方案 A（危险）：在渲染进程开启 nodeIntegration
┌──────────────────────────────────┐
│  渲染进程                         │
│  nodeIntegration: true           │
│  → 可以 require('fs')            │
│  → XSS 也可以 require('child_process').exec('rm -rf /') │
└──────────────────────────────────┘

✅ 方案 B（安全）：用 preload 脚本做安全桥梁
┌──────────────────────────────────┐
│  渲染进程                         │
│  nodeIntegration: false（默认）   │
│  不能 require('fs')              │
│  但可以通过 window.electronAPI   │
│  调用 preload 暴露的安全方法      │
└──────────────────────────────────┘
         ↑ contextBridge
┌──────────────────────────────────┐
