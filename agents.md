...
| [english-lesson-crafter/](.agents/skills/english-lesson-crafter/SKILL.md)             | "写 L02"、"继续下一讲"、"生成课文"、"重写本讲" | 通用英语三册课文生成工作台（v0.1）：大纲定位 → 词汇台账查重（精讲唯一 + 自然复现）→ 六节模板成文 → exam-item-toolkit 规范命题 → 台账回写 → 五查自审（深度审查联动 doc-quality-reviewer） |
